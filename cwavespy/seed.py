...
def gen_new_seed(length=15):
    words_count = len(_words)
    words = [_words[secrets.randbelow(words_count)] for i in range(length)]
    phrase = ' '.join(words)
    if len(phrase) < _MIN_SEED_LEN:
        raise Exception("The resulted seed length is less than the minimum length (%d)" % _MIN_SEED_LEN)
    return phrase
